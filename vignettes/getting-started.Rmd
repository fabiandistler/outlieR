...
`outlieR` is optimized for efficient work with `data.table`:
...
```{r eval=FALSE}
# Understand data before outlier detection
summary(your_data)
plot(your_data) # Base R plots
cor(your_data[, numeric_cols]) # Correlations
```

## 2. Adjust the contamination Parameter
...
# Then adjust iteratively based on results
```
...
# Feature importance is particularly useful
plot(result, type = "features")
```
...
# Are the outliers meaningful?
```

# Summary

`outlieR` provides a complete solution for outlier detection:

* ✅ **Simple API**: One function for most cases
* ✅ **Automatic tuning**: Or manual control
* ✅ **Detailed diagnostics**: Feature-level analysis
* ✅ **Visualizations**: Multiple plot types
* ✅ **data.table integration**: Fast and flexible
* ✅ **Reproducible**: Seed support

For more information, see:

* `?detect_outliers` for full parameter documentation
* [GitHub Issues](https://github.com/yourusername/outlieR/issues) for questions
* [Vignettes](https://github.com/yourusername/outlieR/vignettes) for more examples
